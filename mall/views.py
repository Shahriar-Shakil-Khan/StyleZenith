...
def home(request):
    featured_products = Product.objects.filter(available=True).order_by('-created')[:500]
...
def product_list(request, category_slug=None):
    ...
    if request.GET.get('search'):
        query = request.GET.get('search')  #when we search any product lower case chinta koei search kore
...
def cart_detail(request):
    ...
    return render(request, 'mall/cart.html', {'cart' : cart})
...
def cart_add(request, product_id):
    ...
    return redirect('mall:product_detail', slug=product.slug)
...
def checkout(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            for item in cart.items.all():  #when we checkout and cart delete
            ...
            return redirect('mall:payment_process')
    else:
        initial_data = {}
...
def payment_success(request, order_id):
    ...
    return render(request, 'mall/payment_success.html', {'order': order})
...
def rate_product(request, product_id):
    ...
    return render(request, 'mall/rate_product.html', {
        'form' : form,
        'product' : product
    })
...
def cancel_order(request, order_id):
    ...
    if order.status == 'processing':
        ...
        if order.paid and order.transaction_id:
            # Replace with your SSLCOMMERZ refund API endpoint and credentials
            refund_url = "https://sandbox.sslcommerz.com/api/v1/refund-request/"
            payload = {
                "store_id": "style6898107c1648d",
                "store_passwd": "style6898107c1648d@ssl",
                "refund_amount": order.get_total_cost(),
                "bank_tran_id": order.transaction_id,
                "refund_remarks": "Order canceled by user"
            }
            response = requests.post(refund_url, data=payload)
            if response.status_code == 200 and response.json().get("status") == "SUCCESS":
                order.status = 'canceled'
                order.save()
                messages.success(request, "Order canceled and payment refunded.")
            else:
                messages.error(request, "Order canceled, but refund failed. Please contact support.")
        else:
            order.status = 'canceled'
            order.save()
